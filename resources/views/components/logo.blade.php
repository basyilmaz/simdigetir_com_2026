...
@php
    ...
    $s = $sizes[$size ?? 'md'];
    $logoUrl = \Modules\Settings\Models\Setting::getValue('brand.logo_url', '');
    $logoAlt = \Modules\Settings\Models\Setting::getValue('brand.logo_alt', 'SimdiGetir');
    $heightOverride = \Modules\Settings\Models\Setting::getValue('brand.logo_height_'.($size ?? 'md'), null);
@endphp
...
<a href="/" class="simdigetir-logo" style="display:inline-flex; align-items:center; gap:{{ $s['gap'] }}px; text-decoration:none;">
    @if(!empty($logoUrl))
        <img
            src="{{ $logoUrl }}"
            alt="{{ $logoAlt }}"
            style="height: {{ is_numeric($heightOverride) ? (int) $heightOverride : $s['height'] }}px; width: auto; display: block;"
            loading="lazy"
        />
    @else
        {{-- Icon --}}
